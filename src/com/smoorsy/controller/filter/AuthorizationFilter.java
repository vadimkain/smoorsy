...
public class AuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        boolean accessToPUBLIC = UNKNOWN.stream().anyMatch(requestURI::equals) && !isUserLoggedIn(servletRequest);
        boolean accessToLEARNER = LEARNER.stream().anyMatch(requestURI::equals) && roles(servletRequest).stream().anyMatch(usersAndRoles -> usersAndRoles.getRole().equals(Role.LEARNER));
        boolean accessToTEACHER = TEACHER.stream().anyMatch(requestURI::equals) && roles(servletRequest).stream().anyMatch(usersAndRoles -> usersAndRoles.getRole().equals(Role.TEACHER));
        boolean accessToCLASSROOM_TEACHER = CLASSROOM_TEACHER.stream().anyMatch(requestURI::equals) && roles(servletRequest).stream().anyMatch(usersAndRoles -> usersAndRoles.getRole().equals(Role.CLASSROOM_TEACHER));
        boolean accessToMANAGER = MANAGER.stream().anyMatch(requestURI::equals) && roles(servletRequest).stream().anyMatch(usersAndRoles -> usersAndRoles.getRole().equals(Role.MANAGER));
...
//        Загружаем статику независимо от роли пользователя
        ...
        if (accessToPUBLIC) filterChain.doFilter(servletRequest, servletResponse);
    ...
    private List<Users_and_Roles> roles(ServletRequest servletRequest) {
        if (isUserLoggedIn(servletRequest)) {
            UserDto user = (UserDto) ((HttpServletRequest) servletRequest).getSession().getAttribute("USER");
            return userService.checkRoles(user);
        }
